...
```
In table 5, I compare regression estimates of $Y$ on $X$ with three different standard errors. The naive variance estimate is much smaller than the robust and bootstrapped SEs. The robust SE is slightly larger which is to be expected and the bootstrapped SE is slightly smaller than the robust SE. Based on these reults, we can conclude that the bootstrapped SE recovers the robust Huber-White estimate of the variance rather than the $i.i.d.$ estimate. 
